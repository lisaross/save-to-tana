 ...
 */
export function extractContentForTana(doc) {
    ...
    function processNode(node) {
        ...
        if (/^H[1-6]$/.test(node.tagName)) {
            children.push({
                type: 'heading',
                attributeId: '',
                children: [{ name: node.textContent || '' }],
            });
        }
        else if (node.tagName === 'UL' || node.tagName === 'OL') {
            // Handle lists
            const listItems = Array.from(node.children).filter((el) => el.tagName === 'LI');
            children.push({
                type: 'list',
                attributeId: '',
                children: listItems.flatMap((li) => processNode(li)).flatMap(liChild => liChild.children),
            });
        }
            ...
            children.push({
                type: 'listItem',
                attributeId: '',
                children: [{ name: getFormattedText(node) }],
            });
        }
    ...
    const rootNode = {
        ...
        supertags: [],
        children: processNode(main),
    };
    return [rootNode];
}
//# sourceMappingURL=extractContentForTana.js.map